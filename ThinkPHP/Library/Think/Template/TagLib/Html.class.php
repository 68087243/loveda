...
class Html extends TagLib{
    ...
    protected $tags   =  array(
        ...
        'cache'     => array('attr'=>'key,time','close'=>1),
    ...
    public function _cache($attr,$content){
        ...
        $tag        =   $this->parseXmlAttr($str,'cache');
        $key  =   $this->parseCondition($tag['key']);
        $time =   $this->parseCondition($tag['time']);
        $time?$time:3600;
        if(preg_match('/[\$\(\)\[\]]/',$key)){  //php 代码
            $parseStr = '<?php  $extendCacheTmp = pageCache('.$key.'); if($extendCacheTmp){ echo $extendCacheTmp; }else{ $extendFirstTmp=ob_get_contents();ob_clean();ob_start(); ?>'
                .$content.'<?php $extendTwoTmp = ob_get_contents();ob_clean(); pageCache('.$key.',$extendTwoTmp,'.$time.'); echo $extendFirstTmp.$extendTwoTmp; } ?>';
        }else {
            $parseStr = '<?php  $extendCacheTmp = pageCache("'.$key.'"); if($extendCacheTmp){ echo $extendCacheTmp; }else{ $extendFirstTmp=ob_get_contents();ob_clean();ob_start(); ?>'
                .$content.'<?php $extendTwoTmp = ob_get_contents();ob_clean(); pageCache("'.$key.'",$extendTwoTmp,'.$time.'); echo $extendFirstTmp.$extendTwoTmp; } ?>';
        }
