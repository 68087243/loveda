...
class Email
{
	...
	{
		$this->debug = false;
		$this->smtp_port = C('config.SMTP_PORT');
		$this->relay_host = C('config.SMTP_SERVER');
		$this->time_out = 30;
		$this->auth = true;
		$this->user = C('config.SMTP_USERNAME');
		$this->pass = C('config.SMTP_USERPWD');
		$this->host_name = C('config.SMTP_SERVER'); 
		$this->log_file ="";
		$this->sock = FALSE;
	}
	...
	{ 
		if(is_array($data))
		{
			$data['mailtype']  = empty($data['mailtype'])? 'HTML':$data['mailtype'];
			$data['mailfrom']  = empty($data['mailfrom'])? C('config.SMTP_USERNAME'):$data['mailfrom'];
