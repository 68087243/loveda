...
/**
 * 页面缓存 服务于 扩展标签 <extend:cache></extend:cache>
 */
function pageCache($name,$value='',$time=3600){
    if(C('PAGE_CACHE_REDIS_DB')){
        $redis = \Think\Cache\Driver\Redis::getInstance('',array("db"=>C('PAGE_CACHE_REDIS_DB')));
    }else{
    ...
    if(''=== $value){ // 获取缓存
        ...
        $expire     =   is_numeric($time)?$time:3600;
