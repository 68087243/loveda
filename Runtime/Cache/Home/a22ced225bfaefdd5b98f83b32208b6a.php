...
<head lang="en">
    <meta charset="UTF-8">
    <title><?php echo ($plan["title"]); ?></title>
...
<div class="container" style="margin-top: 30px;padding: 0 0 50px 0 ;">
    <div class="fl column" style="width: 910px;" id="content" class="bordert-d" >
        <div class="topic-itme "  style="width: 850px;margin:40px 0;border-bottom: 1px dashed #cccccc;">
            <div>
                <div class="fl" style="width:100px;">
                    <img id="picture" src="http://www.lovehou.com/Public<?php echo ((isset($plan["avatar"]) && ($plan["avatar"] !== ""))?($plan["avatar"]):'/static/image/noavatar_middle.gif'); ?>" width="80" /><br/>
                    <div><?php echo ($plan["nickname"]); ?></div>
                </div>
                <div class="fl" style="width: 700px;">
                    <div class="fontb font18"><?php echo ($plan["title"]); ?></div>
                    <div><?php echo timeTran($plan['createtime']);?></div>
                </div>
                <div class="clr"></div>
            </div>
            <div class="topic_content font14 clo6 " style="padding-bottom:30px ;"  >
                <?php echo ($plan["contentcn"]); ?>
                <?php if($plan['sex'] != 1): if($plan['img']): if(is_array($vo['img'])): $i = 0; $__LIST__ = $vo['img'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$img): $mod = ($i % 2 );++$i;?><img src="http://www.lovehou.com/Public/<?php echo ($img); ?>" style="max-width: 100%"  alt=""/><?php endforeach; endif; else: echo "" ;endif; endif; ?>
                    <?php else: ?>
        ...
        <div class="reply-box" style="border: 2px solid #BCEFEB">
            <div style="width: 860px;margin: 20px;">
                ...
                <div class="reply-input" id="reply">
                    <input type="hidden" name="pid" value="<?php echo ($plan["pid"]); ?>"/>
                    <input type="hidden" name="uid" value="<?php echo ($plan["uid"]); ?>"/>
                    <input type="hidden" name="touser" value="<?php echo ($plan["nickname"]); ?>"/>
                    <textarea name="message" style="border-color: #cccccc;width: 860px;height: 50px;resize: none;margin-top: 20px;" id="" placeholder="温馨提示：系统自动删除包含联系方式，色情低俗，重复等信息的评论">
                    </textarea>
        ...
        <div class="reply-box borderb-d" style="border: 2px solid #BCEFEB;margin-top: 10px;">
            <ul class="" style="width: 860px;margin: 20px;">
                <?php if(is_array($reply)): $i = 0; $__LIST__ = $reply;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="reply-item bordert-d">
                        <div style="margin: 5px">
                            <a href="/user/userinfo.html?uid=<?php echo ($vo["uid"]); ?>" class="item-user">
                                <img src="http://www.lovehou.com/Public<?php echo ((isset($plan["avatar"]) && ($plan["avatar"] !== ""))?($plan["avatar"]):'/static/image/noavatar_middle.gif'); ?>" width="40" />
                                <span class="font18 clo6"><?php echo ($vo["nickname"]); ?></span>
                                <span class="fr font12 clo9"><?php echo timeTran($vo['createtime']);?></span>
                            </a>
                        </div>
                        <div class="clo9" style="background:#F1FBFD;border: 1px solid #BFD2DC;margin: 5px 0 10px 0; "><?php echo ($vo["message"]); ?>
                            <?php if($currentuid != $vo['uid']): ?><div class="fr relative top5">
                                    <a class="green" href="javascript:reply('<?php echo ($vo["nickname"]); ?>','<?php echo ($vo["uid"]); ?>')">回复</a>
                                    <a class="green" href="javascript:reply('<?php echo ($vo["id"]); ?>','<?php echo ($vo["uid"]); ?>')">支持(<?php echo ((isset($vo["hot"]) && ($vo["hot"] !== ""))?($vo["hot"]):0); ?>)</a>
                                </div><?php endif; ?>
                        </div>
    ...
    <div class="fr column" style="width: 295px;">
        <div class="panel panel-default">
            <div class="panel-heading" style="background: #7EE0D9" >
                <span class="panel-title fontb clo3">最新计划</span>
                <div class="clr"></div>
            </div>
            <div class="panel-body" style="background: #F9F9F9">
                <ul class="clo6">
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="borderb-d" style="padding: 5px 0;">
                            <div> <a href="/index/plan.html?pid=<?php echo ($vo["pid"]); ?>" class="clo3"><?php echo (msubstr($vo['title'],0,25)); ?></a></div>
                            <div class="font12 clo6" style="height: 20px;;"><?php echo timeTran($vo['createtime']);?></div>
                        </li><?php endforeach; endif; else: echo "" ;endif; ?>
                </ul>
            </div>
        </div>
...
<script>
    ...
    //提交评论
    function subReply(){
        var pid = $('#reply input[name=pid]').val();
        ...
        $.post('/index/subPlanReply.html',data,function(data){
            if(data.code==200){
