        ...
        <div class="topic-itme "  style="width: 850px;margin:40px 0;border-bottom: 1px dashed #cccccc;">
            <div>
                <div class="fl" style="width:100px;">
                    <img id="picture" src="http://www.lovehou.com/Public<?php echo ((isset($topic["avatar"]) && ($topic["avatar"] !== ""))?($topic["avatar"]):'/static/image/noavatar_middle.gif'); ?>" width="80" /><br/>
                    <div><?php echo ($topic["nickname"]); ?></div>
                    <!--<div><?php if($topic['city']): echo getAreaByAid($topic['city']); elseif($topic['proivce']): echo getAreaByAid($topic['proivce']); endif; ?></div>-->
                </div>
                <div class="fl" style="width: 700px;">
                    <div class="fontb font18"><?php echo ($topic["title"]); ?></div>
                    <div>
                        <a href="javascript: likeTopic('<?php echo ($topic["tid"]); ?>');">点赞</a>(<?php echo ((isset($topic["likeit"]) && ($topic["likeit"] !== ""))?($topic["likeit"]):0); ?>) &nbsp;
                        <a href="javascript: favorite('<?php echo ($topic["tid"]); ?>');">收藏</a>  &nbsp;
                        <a href="javascript: $('#reply textarea[name=message]').focus();">评论</a>(<?php echo ((isset($topic["comments"]) && ($topic["comments"] !== ""))?($topic["comments"]):0); ?>)
                    </div>
                    <div><?php echo timeTran($topic['createtime']);?></div>
                </div>
                <div class="clr"></div>
            </div>
            <div class="topic_content font14 clo6 " style="padding-bottom:30px ;"  >
                <?php echo ($topic["message"]); ?>
                <?php if($topic['sex'] != 1): if($topic['img']): if(is_array($vo['img'])): $i = 0; $__LIST__ = $vo['img'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$img): $mod = ($i % 2 );++$i;?><img src="http://www.lovehou.com/Public/<?php echo ($img); ?>" style="max-width: 100%"  alt=""/><?php endforeach; endif; else: echo "" ;endif; endif; ?>
                    <?php else: ?>
        ...
        <div class="reply-box" style="border: 2px solid #BCEFEB">
            <div style="width: 860px;margin: 20px;">
                ...
                <div class="reply-input" id="reply">
                    ...
                    <input type="hidden" name="touser" value="<?php echo ($topic["nickname"]); ?>"/>
                ...
                <?php if(is_array($reply)): $i = 0; $__LIST__ = $reply;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="reply-item bordert-d">
                        <div style="margin: 5px">
                            <a href="/user/userinfo.html?uid=<?php echo ($vo["uid"]); ?>" class="item-user">
                                <img src="http://www.lovehou.com/Public<?php echo ((isset($topic["avatar"]) && ($topic["avatar"] !== ""))?($topic["avatar"]):'/static/image/noavatar_middle.gif'); ?>" width="40" />
    ...
    <div class="fr column" style="width: 295px;">
        <div class="panel panel-default">
            <div class="panel-heading" style="background: #7EE0D9" >
                <span class="panel-title fontb clo3">热门文章</span>
                <div class="clr"></div>
            </div>
