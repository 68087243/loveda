...
<body>
	<table border="0" cellpadding="3" cellspacing="1" class="MainTbl">
		<tr class="toolbar">
			<td colspan="6" class="tc">【 管理用户 】</td>
		</tr>
		<tr class="row0">
			<td colspan="6"><a href="<?php echo U('Admin/Rbac/addUser');?>"
		...
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="row<?php echo ($i % 2+1); ?>"> 
			...
			<td><input name="Item_1" id="Item_1"
				onchange="setVal('User','sort',<?php echo ($vo["id"]); ?>,$(this).val())"
				class="inputText1 numeric w50" value="<?php echo ($vo["sort"]); ?>" /></td>
			<td><?php if(($vo["status"] == 1)): ?><a
					href="javascript:void(0);"
					onclick="setVal('User','status',<?php echo ($vo["id"]); ?>,0,this,'禁用')">启用</a> <?php else: ?>
				<a href="javascript:void(0);"
					onclick="setVal('User','status',<?php echo ($vo["id"]); ?>,1,this,'启用')">禁用</a><?php endif; ?></td>
			<td>  <a
				href="<?php echo U('Rbac/editUser','id='.$vo['id']);?>" class="btnEdit">修改</a>
				<?php if(($vo['id'] == 2)): else: ?>
				
				<a class="btnDel" href="javascript:void(0);"
				onclick="var url='<?php echo U("Rbac/deleteUser","id=".$vo['id']);?>';if(confirm('您确定删除该记录吗？')){location=url;}">删除</a></td><?php endif; ?>
