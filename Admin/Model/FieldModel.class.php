...
class FieldModel extends Model {
	...
	protected function checkTableExist($model_id) {
		...
		$table_name = $this->table_name = C ( 'DB_PREFIX' ) . 'content_' . strtolower ( $model ['name'] );
...
sql;
			...
			// 新建表时是否默认新增“id主键”字段 AUTO_INCREMENT
...
sql;
	 ...
	 */
	protected function getNewName($tbl='', $b = true) {
		if ($b) {
			$tbl = (C ( 'MODEL_PREFIX' ) ? 'ext_' : C ( 'MODEL_PREFIX' )) . $tbl;
		} else {
			$tbl = str_replace ( (C ( 'MODEL_PREFIX' ) ? 'ext_' : C ( 'MODEL_PREFIX' )), '', $tbl );
		}
		return $tbl;
	}
}
